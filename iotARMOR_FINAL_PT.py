...
class TopLevelWindow(ctk.CTkToplevel):
    def __init__(self, *args, **kwargs):
        ...
        self.ml_button = ctk.CTkButton(self, fg_color="dark green" ,command=self.close, text="Exit")
        self.ml_button.place(x=0, y=650)
        self.ml_button.configure(font=('Nirmala UI', 30), height=100, width=400)

        self.active_button = ctk.CTkButton(self, fg_color="seagreen", command=self.button_activate,
                                           text="Start Monitoring")
        self.active_button.place(x=0, y=100)
        self.active_button.configure(font=('Nirmala UI', 30), height=100, width=400)

        self.no_active_button = ctk.CTkButton(self, fg_color="seagreen", command=self.no_button_activate,
        ...
        self.start_button = ctk.CTkButton(self, fg_color="forestgreen", command=self.start_capture,
        ...
        self.stop_button = ctk.CTkButton(self, fg_color="forestgreen", command=self.stop_capture,
        ...
        self.close_button = ctk.CTkButton(self, fg_color="darkseagreen", command=self.start_machine_learning,
        ...
        self.my_frame = ctk.CTkScrollableFrame(self, orientation="vertical", width=350, height=420, 
                                               label_text="Active IP Addresses", label_text_color="magenta2", label_font=('Berlin Sans FB', 30))
        self.my_frame.place(x=1000, y=100)

        #for x in range(20):
            #ctk.CTkLabel(self.my_frame, text="IP Address!").pack(pady=10)

        self.void = ctk.CTkLabel(self, text="STATUS", text_color= "turquoise", font=('Consolas', 70))
        self.void.place(x=610, y=400)
        self.status_label = ctk.CTkLabel(self, text=" ", text_color="deepskyblue1", font=('Terminal', 30))
        self.status_label.place(x=510, y=530)

        # this is the part you are looking for with the user input
        self.iplabel = ctk.CTkLabel(self, text="Please enter your IP addresses here Ex: 123.123.123.0/24", text_color= "aquamarine", width=120,
                                    height=25, corner_radius=20, font=('Nirmala UI', 20))
        self.iplabel.place(x=490, y=40)
        ...
        self.image_path = os.path.join(os.path.dirname(__file__), 'IOT ARMOR LOGO.png')
        self.image_label = ctk.CTkLabel(self, image=Image.open(self.image_path))
    ...
    def do_colors(self, safe_ips, bad_ips):
        for label in self.ip_labels:
            ...
            if ip in safe_ips:
                ...
                label.configure(text_color="green")  # IPs that are neutral


    def button_activate(self):
        self.status_label.configure(text="Starting Monitoring...")
    ...
    def no_button_activate(self):
        ...
        self.active_button.configure(state=tk.NORMAL)
        self.stop_event.set()
...
class App(ctk.CTk):
    ...
    def login(self):
        self.username = "Spartans"
        self.password = "greenandgold1935!"
